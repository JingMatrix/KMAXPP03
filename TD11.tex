...
\begin{enumerate}
	...
	\begin{enumerate}
        ...
        Alors, par le second lemme de Borel--Cantelli (par indépendance des $X_n$), on en déduit que, p.s., il y a une infinité de $n$ tels que $X_n \geq \varepsilon$, donc, p.s., $X_n$ ne tend pas vers 0. 
        %%
		\item Supposons que $\lambda_n/\log n \to \infty$.
		Soit $\varepsilon > 0$.
        Alors, pour tout $n$ suffisamment grand, on a $\lambda_n \geq \frac{2}{\varepsilon} \log n$ et donc $e^{-\lambda_n \varepsilon} \leq n^{-2}$, qui est sommable. Donc on a bien
		\[
		\sum_{n\geq 1} e^{-\lambda_n \varepsilon} < \infty,
		\]
		ce qui montre la convergence p.s. par 3.(a).
		%%
		\item Supposons $(\lambda_n/\log n)_{n\geq 2}$ bornée : il existe $C>0$ tel que $\lambda_n/\log n \leq C$ pour tout $n \geq 2$.
		On a alors
		\[
		...
		= \infty.
		\]
        donc il n'y a pas convergence p.s. par 3.(a).
	\end{enumerate}
\end{enumerate}
\end{comment}
