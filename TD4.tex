...
\begin{exo}
	...
	et $Z = \1_B$ où $B$ est l'événement ``le 1er dé donne un résultat pair''.
	\begin{enumerate}
...
\begin{comment}
\begin{enumerate}
	\item On prend $\Omega = \llbracket 1,6 \rrbracket^2$ muni de $\cA = \cP(\Omega)$ et de la mesure uniforme $\P$, i.e. $\P(A) = \abs{A}/36$ pour $A \in \cA$.
	Alors $X(i,j) = i+j$ et $Y(i,j) = \abs{i-j}$ pour $(i,j) \in \Omega$. Puis
	\[
		B = \{(i,j) \in \Omega : i \text{ est pair}\}
	\]
	et $Z=\1_B$.
	%%
	\item Notons tout d'abord que $Z$ ne prend que les valeurs 0 et 1 donc sa loi est caractérisée par $\P(Z=1) = \P(B) = 1/2$ (en énumérant les éléments de $B$).
	...
	\begin{align*}
	\P(X=2,Z=1) & = \P(\{(1,1)\} \cap B) = \P(\varnothing) = 0
