...
\end{cases}
\]
La fonction $F_Y$ est $\cC^1$ sur $\R \setminus \{0,1\}$, donc par la Proposition 1.4.5, avec $F'$ qui est définie Lebesgue-presque partout, on a 
\begin{align*}
\diff P_Y(x) 
& = F'(x) \diff x + (F(0)-F(0-)) \diff \delta_0(x) + (F(1)-F(1-)) \diff \delta_1(x) \\
...
= - \theta \phi_X(\theta).
\]
\item Les solutions de l'équation différentielle $\phi_X'(\theta) = - \theta \phi_X(\theta)$ sont de la forme $\phi_X(\theta) = C e^{-\theta^2/2}$ avec $C \in \R$ (obtenues en intégrant $\frac{\phi_X'}{\phi_X} (\theta) = - \theta$).
...
\begin{comment}
On a vu à l'exercice \ref{exo:fc_rép} que
\[ 
\P(X < a) = P_X(]-\infty,a[) = F_X(a-).
\]
Puis $\P(X = a) = F_X(a) - F_X(a-)$, $\P(X>a) = 1 - F(a)$, $\P(a \leq X \leq b) = F(b) - F(a-)$ et $\P(a < X < b) = F(b-) - F(a)$.
\end{comment}
