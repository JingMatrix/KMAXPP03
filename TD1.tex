...
\begin{exo}[Mesure image]
	...
	Rappelons que $P_X$ est définie par $P_X(B) = \P(X^{-1}(B))$ pour tout $B \in \cE$. Montrer que $P_X$ est une mesure de probabilité sur $(E,\cE)$.
\end{exo}
...
\begin{comment}
Expliquons d'abord la discussion avec des mots. Dans sa première réponse, Pierre tente d'évaluer la probabilité que Mathilde ait une fille, sachant qu'elle a deux enfants dont un garçon, et il dit que cette probabilité vaut $2/3$. Il apprend ensuite que le garçon de Mathilde est son aîné. Il réévalue la probabilité que Mathilde ait une fille, sachant cette nouvelle information, et obtient $1/2$ (il ne peut plus parier sur une option plus probable que l'autre, donc ``il ne peut plus rien dire'').
...
\begin{exo} Un article du journal Le Monde était consacré au suicide dans les prisons. On y apprenait notamment que le taux de suicide y est d'environ $1/1000$ par an et que, parmi les détenus s'étant suicidés, on trouvait une population ayant déja fait une tentative de suicide au cours des années précédentes 15 fois plus importante que des détenus n'ayant jamais attenté à leur vie. Le Monde concluait à la nécessité de surveiller de près les détenus ayant déjà fait une tentative de suicide.
	\begin{enumerate}
		\item Donner un modèle probabiliste prenant en compte les éventualités précisées plus haut (tentative de suicide, suicide). 
		%%
		\item Exprimer dans ce modèle la conclusion du quotidien et évaluer la justesse de cette conclusion en fonction de la proportion de personnes ayant déja fait une tentative de suicide.
	\end{enumerate}
\end{exo}
