...
\begin{enumerate} 
	...
	\begin{enumerate} 
		...
		\emph{Remarque.} On a utilisé ici que 
		\[
			\limsup_{n\to\infty} a_nb_n
			\leq \left( \limsup_{n\to\infty} a_n \right)
			\left( \limsup_{n\to\infty} b_n \right),
		\]
		qui est vraie dès que le produit à droite a un sens (c'est-à-dire n'est pas une forme indéterminée).
	\end{enumerate}
	%%
...
\begin{exo}[Le cas des variables aléatoires qui ne sont pas dans $L^1$]
	\begin{enumerate}
		...
		\begin{enumerate}
			\item Soit $M > 0$. Pour $n \geq 1$, on pose $Y_n = \max(X_n,M)$.
			Montrer que 
			\[
				\frac{Y_1+\dots+Y_n}{n}
				\xrightarrow[n\to\infty]{\text{p.s.}} \E[\max(X_1,M)].
			\]
			%%
			\item En déduire que
			\[
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\]
		%%
		\item Comme $\E[\abs{X_1}] = \infty$, on sait que $\limsup_{n\to\infty} X_n/n = \infty$ p.s. 
	...
	\begin{enumerate}
		...
		\[
		\frac{Y_1+\dots+Y_n}{n}
		\xrightarrow[n\to\infty]{\text{p.s.}} \E[Y_1] = \E[\max(X_1,M)].
		\]
		%%
		...
		\liminf_{n \to \infty} \frac{X_1+\dots+X_n}{n} \geq \E[\max(X_1,M)].
		\]
		Donc, comme $\E[\max(X_1,M)] \to \E[X_1] = \infty$ par le théorème de convergence monotone (car $\max(X_1,M) \geq 0$ et $\max(X_1,M) \uparrow X_1$ quand $M\to\infty$),
		\[
		\text{p.s.}, \, 
