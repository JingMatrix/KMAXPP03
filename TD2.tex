...
\begin{comment}
\begin{enumerate} 
\item Si $X=Y$ p.s.~alors, pour toute fonction mesurable $f \colon \R\to\R_+$, $f(X)=f(Y)$ p.s. et donc $\Ec{f(X)}=\Ec{f(Y)}$ (car les intégrales par rapport à $\P$ de deux fonctions égales $\P$-presque partout sont égales), ce qui montre que $X$ et $Y$ ont la m\^eme loi : en effet, pour tout $B \in \cB(\R)$, en prenant $f = \1_B$, on a
\[
...
\emphb{Méthode.} Pour déterminer la loi d'une v.a. réelle $Y$, on pourra utiliser la méthode suivante, présentée plus en détails en Section 1.3.3 des notes de cours : considérer une fonction $f \colon \R \to \R_+$ mesurable quelconque et écrire $\E[f(Y)]$ sous la forme $\int_\R f(y) \diff \mu(y)$. Alors on peut en conclure que $P_Y = \mu$.
...
\begin{comment}
\begin{enumerate}
\item Si $Y < X$, alors il existe un rationnel $q \in \Q$ tel que $Y < q \leq X$, par densité de $\Q$ dans $\R$. Donc, on a l'inclusion (c'est en fait une égalité !)
\[
	\{ Y < X \} \subset \bigcup_{q\in\Q} \{Y < q \leq X\}.
\]
Donc
\[
	\P(Y < X) \leq \sum_{q\in\Q} \P(Y < q \leq X) = 0,
\]
en utilisant l'hypothèse de l'énoncé.
On a donc montré que $\mathbb{P}(Y < X)=0$.
%%
\item Supposons que $X$ et $Y$ ont m\^eme loi et que $X \geq Y$ p.s.
Alors, avec $\Omega' = \{X \geq Y\}$, on a $\P(\Omega'^c) = 0$.
Soit $t \in \R$, on a
\[
\{ Y < t \leq X \}
= \{X \geq t\} \setminus \{Y \geq t\}.
\]
Puis on a l'union disjointe
\[
\{X \geq t\} \setminus \{Y \geq t\}
= \left( \{X \geq t\} \cap \Omega'] \setminus [\{Y \geq t\}\cap \Omega'] \right)
\sqcup \left( \{X \geq t\} \cap \Omega'^c] \setminus [\{Y \geq t\}\cap \Omega'^c] \right),
\]
où le 2ème événement de cet union disjointe est inclus dans $\Omega'^c$, donc a probabilité 0. On en déduit que
\[
\P(Y < t \leq X)
= \P([\{X \geq t\} \cap \Omega'] \setminus [\{Y \geq t\}\cap \Omega']).
\]
Notons maintenant que $\{Y \geq t\}\cap \Omega' \subset \{X \geq t\}\cap \Omega'$, donc
\[
\P(Y < t \leq X)
= \P(\{X \geq t\} \cap \Omega') - \P(\{Y \geq t\}\cap \Omega').
\]
Mais $\P(\{X \geq t\} \cap \Omega') = \P(X \geq t) - \P(\{X \geq t\} \cap \Omega'^c) = \P(X \geq t)$ car $\P(\Omega'^c) = 0$.
En procédant de même avec $\P(\{Y \geq t\}\cap \Omega')$, on obtient
\[
\P(Y < t \leq X)
= \P(X \geq t) - \P(Y \geq t) = 0,
\]
car $X$ et $Y$ on la même loi. Par la question 1, on en déduit que $\P(Y < X) = 0$, c'est-à-dire $Y \geq X$ p.s. Mais comme on a aussi $X \geq Y$ p.s., on a donc montré que $X = Y$ p.s.
\end{enumerate}
\end{comment}
...
\begin{comment}
Soit $Y = \lfloor X \rfloor$.
Soit $f \colon \R \to \R_+$ une fonction mesurable.
Par le théorème de transfert,
\[
\Ec{f(Y)} = \Ec{f(\lfloor X \rfloor)}
= \int_0^\infty f(\lfloor x \rfloor) e^{-x} \diff x
= \sum_{k=0}^\infty \int_k^{k+1} f(k) e^{-x} \diff x
= \sum_{k=0}^\infty \left( e^{-k}-e^{-(k+1)} \right) f(k).
\]
On peut y reconnaître l'intégrale suivante
\[
\Ec{f(Y)} 
= \int_\R f(y) \diff \left( \sum_{k=0}^\infty \left( e^{-k}-e^{-(k+1)} \right) \delta_k \right) (y)
\]
et donc
\[
P_Y = \sum_{k=0}^\infty \left( e^{-k}-e^{-(k+1)} \right) \delta_k.
\]
\end{comment}
